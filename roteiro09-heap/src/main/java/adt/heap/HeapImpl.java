...
public class HeapImpl<T extends Comparable<T>> implements Heap<T> {
	...
	private void heapify(int position) {
		...
		int largest = this.getMaxIndex(position, left, right);
	...
	private int getMaxIndex(int root, int left, int right) {
		int max = root;
		if (left < this.size() && this.comparator.compare(this.heap[root], this.heap[left]) < 0) {
			max = left;
		}
		if (right < this.size() && this.comparator.compare(this.heap[max], this.heap[right]) < 0) {
			max = right;
		}
		return max;
	}
